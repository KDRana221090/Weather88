...
public class NVActivityIndicatorPresenter {
    ...
    private func show(with activityData: ActivityData) {
        ...
        activityContainer.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0.75)
        ...
        if let message = activityData.message , !message.isEmpty {
            ...
            if(UIDevice.current.screenType == DeviceScreenType.iPhones_5_5s_5c_SE) {
    ...
    private func hide() {
        if let dataItem = UIApplication.shared.keyWindow?.subviews {
